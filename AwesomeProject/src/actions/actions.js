...
export const getAllPosts = id => (dispatch, getState) => {
    const token = getState().user.token;
    dispatch({ type: types.FETCHING_CLUB_POSTS });
    return API().get(`/getclubposts/${id}`, token).then((data) => {
        dispatch({ type: types.FETCHED_CLUB_POSTS });
        dispatch({
...
export const getAllPoststu = (id) => (dispatch) => {
    dispatch({ type: types.FETCHING_STU_POSTS });
    return API().get(`/getstuposts/${id}`).then((data) => {
        // console.log(data);
        dispatch({ type: types.FETCHED_STU_POSTS });
        dispatch({
...
export const getAllConfs = (id) => (dispatch) => {
    ...
    // }
    // else {
        dispatch({ type: types.FETCHING_CONFS_POSTS });
        API().get(`/getconfs/${id}`).then((data) => {
            // console.log('confs', data);
            dispatch({
                type: types.UPDATE_CONF_REDUCER,
                payload: data
            });
            dispatch({ type: types.FETCHED_CONFS_POSTS });
        }).catch((err) => err && console.log('err', err));
    // }    
};

export const getBasicInfo = () => (dispatch) => {
    dispatch({ type: types.FETCHNG_BASIC_INFO });
    API().get('/basic').then((data) => {
        dispatch({
            type: types.UPDATA_BASIC_INFORMATION,
            payload: data.basic
        });
        dispatch({ type: types.FETCHED_BASIC_INFO });
...
export const searchString = (toSearch) => (dispatch) => {
    dispatch({ type: types.SEARCH_RESULT_IF_FETCHING });
...
export const checkForTheToken = (token) => (dispatch) => {
    if (token) {
        dispatch({
